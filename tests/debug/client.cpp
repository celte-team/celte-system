...
void loadEntitiesFromSummary(const nlohmann::json& summary)
{
    ...
    std::string props = summary["props"];
    ...
    obj->entity->DownloadReplicationData(props);
...
void registerHooks()
{
    ...
    HOOKS.client.player.execPlayerSpawn = [](std::string clientId, int x, int y,
        ...
        if (clientId == RUNTIME.GetUUID())
            Spawned = true;

        return true;
    };

    HOOKS.client.replication.onReplicationDataReceived = [](std::string entityId,
                                                             std::string blob) {
...
void doGameLoop()
{
    ...
    auto duration = std::chrono::duration_cast<std::chrono::seconds>(currentTime - savedTime)
                        .count();
    int wait = 2;
    ...
    if (Spawned && duration > 5) {
        deleteme++;
        game.objects.at(RUNTIME.GetUUID())
            ->entity->sendInputToKafka("move", status);
        status = !status;
        savedTime = std::chrono::steady_clock::now();
        std::cout << RUNTIME.GetUUID() << " is moving forward\n";
        wait++;

        if (deleteme > 4) {
            std::cout << "DISCONNECTING MYSELF\n"
                      << std::endl;
            game.objects.at(RUNTIME.GetUUID())
                ->entity->DisconnectPlayer();
        }
    }
}
