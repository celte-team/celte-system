...
void updateClientsPositions()
{
    ...
    std::string uuidP1 = (*std::next(game.objects.begin(), 0)).second->entity->GetUUID();
    std::cout << "P1 id is : " << uuidP1 << " | P2 id is : " << uuidP2
              << std::endl;
    // auto entity1 = (*std::next(game.objects.begin(), 0)).second->entity;
    ...
    if (last_input_P1 && last_input_P1->back().status) {
        ...
        // if (player->x == 8) {
        //     std::cout << "DELETE A PLAYER\n";
        //     entity1->GetOwnerChunk().ExecDeleteEntity(uuidP1);
        //     t_isNode1 = false;
        // }
    } else {
