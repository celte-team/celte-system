...
namespace celte {
    namespace runtime {
        ...
        std::optional<std::shared_ptr<celte::CelteEntity>>
        CelteEntityManagementSystem::GetEntity(const std::string& uuid) const
        {
            auto res = _entities.find(uuid);
            if (res == _entities.end()) {
                std::cout << uuid << " is not in the ENTITIES map\n";
                return std::nullopt;
            }
            ...
            const std::vector<std::string>& entityIds, const std::string& filter)
        {
            ...
            bool (CelteEntityManagementSystem::*filterMethod)(const std::string&) const = _filters.at(filter);
            ...
            std::unordered_map<std::string, std::string>& data, bool active)
        {
            ...
            for (auto& [entityId, blob] : data) {
                try {
                    ...
                    if (entity)
                        entity.value()->DownloadReplicationData(blob, active);
#ifdef CELTE_SERVER_MODE_ENABLED
                    if (active)
            ...
            const std::string& summary)
        {
            ...
            try {
                ...
                for (nlohmann::json& partialSummary : summaryJSON) {
                    ...
                    if (_entities.find(uuid) != _entities.end() or uuid == RUNTIME.GetUUID()) {
