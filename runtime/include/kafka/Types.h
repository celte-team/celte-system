...
// Which is similar with `boost::const_buffer` (thus avoid the dependency
// towards `boost`)
class ConstBuffer {
public:
  explicit ConstBuffer(const void *data = nullptr, std::size_t size = 0)
      : _data(data), _size(size) {}
  const void *data() const { return _data; }
  std::size_t size() const { return _size; }
  std::string toString() const {
    if (_size == 0)
      return _data ? "[empty]" : "[null]";
 ...
 */
#if COMPILER_SUPPORTS_CPP_17
const inline std::chrono::milliseconds InfiniteTimeout =
    (std::chrono::milliseconds::max)();
#else
const static std::chrono::milliseconds InfiniteTimeout =
    (std::chrono::milliseconds::max)();
#endif
...
inline std::string toString(const Topics &topics) {
  std::string ret;
  std::for_each(topics.cbegin(), topics.cend(), [&ret](const auto &topic) {
...
inline std::string toString(const TopicPartitions &tps) {
  std::string ret;
  std::for_each(tps.cbegin(), tps.cend(), [&ret](const auto &tp) {
    ret.append((ret.empty() ? "" : ",") + tp.first + "-" +
...
inline std::string toString(const TopicPartitionOffset &tpo) {
  return std::get<0>(tpo) + "-" + std::to_string(std::get<1>(tpo)) + ":" +
...
inline std::string toString(const TopicPartitionOffsets &tpos) {
  std::cerr
      << "toString(const TopicPartitionOffsets& tpos) defined in " << __FILE__
      << "is likely a trap and doesnt do what you think it does. :) convert "
         "manually suing record.value().data() and record.value.size()"
      << std::endl;
  std::string ret;
  std::for_each(tpos.cbegin(), tpos.cend(), [&ret](const auto &tp_o) {
    const TopicPartition &tp = tp_o.first;
    const Offset &o = tp_o.second;
    ret.append((ret.empty() ? "" : ",") + tp.first + "-" +
               std::to_string(tp.second) + ":" + std::to_string(o));
  });
  return ret;
}
