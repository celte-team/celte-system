 ...
 * This also consists of a topic name and a partition number from which the
 * record is being received, an offset that points to the record in a Kafka
 * partition
 */
class ConsumerRecord {
public:
  // ConsumerRecord will take the ownership of msg (rd_kafka_message_t*)
  explicit ConsumerRecord(rd_kafka_message_t *msg)
      : _rk_msg(msg, rd_kafka_message_destroy) {}

  // Use this constructor for unit tests
  ConsumerRecord(rd_kafka_message_t *msg,
                 std::function<void(rd_kafka_message_t *)> deleter)
      : _rk_msg(msg, deleter) {}
  ...
  Timestamp timestamp() const {
    ...
    const Timestamp::Value tsValue =
        rd_kafka_message_timestamp(_rk_msg.get(), &tstype);
   ...
   *     - RD_KAFKA_RESP_ERR__PARTITION_EOF,   -- reached the end of a partition
   * (got no message)
   *   2. Failure
   *     - [Error Codes]
...
inline Headers ConsumerRecord::headers() const {
  Headers headers;

  rd_kafka_headers_t *hdrs = nullptr;
  if (rd_kafka_message_headers(_rk_msg.get(), &hdrs) !=
  ...
  for (std::size_t i = 0;
       !rd_kafka_header_get_all(hdrs, i, &name, &valuePtr, &valueSize); i++) {
...
inline Header::Value ConsumerRecord::lastHeaderValue(const Header::Key &key) {
  rd_kafka_headers_t *hdrs = nullptr;
  if (rd_kafka_message_headers(_rk_msg.get(), &hdrs) !=
      RD_KAFKA_RESP_ERR_NO_ERROR) {
    return Header::Value();
  ...
  const void *valuePtr = nullptr;
  std::size_t valueSize = 0;
  return (rd_kafka_header_get_last(hdrs, key.c_str(), &valuePtr, &valueSize) ==
          RD_KAFKA_RESP_ERR_NO_ERROR)
             ? Header::Value(valuePtr, valueSize)
             : Header::Value();
}

inline std::string ConsumerRecord::toString() const {
  std::ostringstream oss;
  if (!error()) {
    oss << topic() << "-" << partition() << ":" << offset() << ", "
        << timestamp().toString() << ", "
        << (key().size() ? (key().toString() + "/") : "") << value().toString();
  } else if (error().value() == RD_KAFKA_RESP_ERR__PARTITION_EOF) {
    oss << "EOF[" << topic() << "-" << partition() << ":" << offset() << "]";
  } else {
    oss << "ERROR[" << error().message() << ", " << topic() << "-"
        << partition() << ":" << offset() << "]";
  }
  return oss.str();
}
