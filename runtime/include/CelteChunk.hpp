...
#define TIMESTAMP std::chrono::time_point<std::chrono::system_clock>
        ...
        public:
            ...
            /**
             * @brief Registers a function that will be called to determine if
             * a player can make spawn this entity.
             * This function will be called by a setter in the godot api
             * and used by the godot developper.
             */
            void registerEntityChecker(std::string& name, std::function<bool(std::string, nlohmann::json& args)> f)
            {
                _entityChecker[name] = f;
            }
            ...
            /**
             * @brief Sends the RPC to delete an entity,
             * only the server can give that order.
             */
            void ExecDeleteEntity(std::string entityId);

            // /**
            //  * @brief Sends the RPC to spawn an entity,
            //  * only the server can give that order after checking the permission.
            //  */
            // void ExecSpawnEntity(std::string clientId, std::string entityName, nlohmann::json& args, std::string tempID, TIMESTAMP timestamp);
        ...
        private:
            ...
            std::map<std::string, std::function<bool(std::string, nlohmann::json& args)>> _entityChecker;
