...
namespace celte {
    namespace net {
        ...
        RPCService::RPCService(const RPCService::Options& options)
            ...
            , _options(options)
            , _writerStreamPool(
                  WriterStreamPool::Options { .idleTimeout = std::chrono::milliseconds(10000) },
                  _io)
        {
        ...
        void RPCService::__handleRemoteCall(const RPRequest& req)
        {
            try {
                ...
                std::cout << "ARGS: " << req.args << std::endl
                          << std::flush;
                auto f = it->second;
        ...
        void RPCService::__handleResponse(const RPRequest& req)
        {
            try {
                ...
                std::cout << "ERROR IN RPR RESPONSE\nKEY: " << req.name << "\nRES: " << e.what() << std::endl;
            }
        }
