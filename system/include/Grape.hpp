...
struct Grape : public CustomRPCTemplate {
  ...
  bool Ping();
  void CMIInstantiate(std::string cmiId, std::string prefabId,
                      std::string payload, std::string clientId);
  void YggdrasilOrderTransfer(std::vector<std::string> entityIds,
...
REGISTER_SERVER_RPC(Grape, YggdrasilOrderTransfer);
REGISTER_RPC(Grape, ExecClientDisconnect);
REGISTER_RPC(Grape, RPCHandler);

} // namespace celte

// 1 binf handle func 'check dans la liste et execute)
// 2 binf register func add to the map
// REGISTER RPC (grape, func "handle") -> call le rpc registered
