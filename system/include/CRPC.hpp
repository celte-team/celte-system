...
namespace celte {
    namespace detail {
        ...
        public:
            ...
            using producer_accessor = tbb::concurrent_hash_map<std::string, ProducerBucket>::accessor;
...
#define DEFINE_TYPE_IDENTIFIER(type)                \
    template <>                                     \
    struct celte::TypeIdentifier<type> {            \
    ...
    public:
        CRPCTimeoutException(std::string message)
    ...
    public:
        ...
        public:
            ...
            void call_async(std::function<void(RetVal)> callback, Args... args)
            {
                ...
                RUNTIME.ScheduleAsyncTask([self = *this, callback, args...]() mutable {
                    ...
                    std::optional<RetVal> result = self.template __call_impl<RetVal>(std::move(uuid), args...);
                    if (result.has_value()) {
        ...
        private:
                ...
                Args&&... args)
            {
                if (_called_on_multiple_peers)
                    throw std::logic_error("Cannot call get() on a call to multiple peers");
                std::variant<std::exception_ptr, std::future<nlohmann::json>> r = RPCCallerStub::instance().call(
                ...
                if (_timeout.count() > 0) {
                    std::future_status f_status = std::get<std::future<nlohmann::json>>(r).wait_for(
                        std::chrono::milliseconds(_timeout));
    ...
    public:
            ...
            const std::string& method_name, Func&& method)
        {
            ...
            auto call_delegate = FunctionClassifier<RetVal, ArgsTuple>::build_delegate(instance, method);
...
#define REGISTER_RPC_CALL_STUB(bound_class, method_name)                                              \
    ...
    template <>                                                                                       \
    struct TypeIdentifier<Call##bound_class##method_name> {                                           \
    ...
    struct Call##bound_class##method_name                                                             \
        ...
        struct Options {                                                                              \
            ...
            std::function<void(celte::CStatus&)> fail_callback = [](celte::CStatus&) {                \
                LOGERROR(                                                                             \
                    "RPC failed : " + celte::TypeIdentifier<Call##bound_class##method_name>::name()); \
                std::cerr                                                                             \
                    << "\033[1;31mRPC failed : "                                                      \
