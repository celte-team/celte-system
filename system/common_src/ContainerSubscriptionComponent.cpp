    ...
    bool isLocallyOwned)
{
    ...
    if (wasCreated) {
        ContainerRegistry::GetInstance().RunWithLock(
            id, [onReady, &id, &containerId, isLocallyOwned](ContainerRegistry::ContainerRefCell& cell) {
                cell.IncRefCount();
    ...
    const std::string& containerId)
{
    ...
    ContainerRegistry::GetInstance().RunWithLock(
        containerId,
        [containerId](ContainerRegistry::ContainerRefCell& cell) { std::cout<< "UNSUB FROM " << containerId  << std::endl; cell.DecRefCount(); });
