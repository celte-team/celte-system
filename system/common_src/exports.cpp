 ...
 *
 * This function fetches the grape ID from the runtime environment and returns
 * it as a string. It is available only when CELTE_SERVER_MODE_ENABLED is
 * enabled.
 *
...
PopCMIInstantiate(const std::string& grapeId)
{
    std::optional<std::tuple<std::string, std::string, std::string, std::string>>
        result = GRAPES.PopNamedTaskFromEngine<std::string, std::string, std::string,
...
{
    std::cout << "In export, Registering global RPC " << name << " with filter " << filter << std::endl;
    if (filter == 0)
...
EXPORT void RegisterClientRPC(const std::string& clientId,
    const std::string& name,
    std::function<void(std::string)> f)
{
