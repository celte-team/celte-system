...
void ETTRegistry::RegisterEntity(Entity&& e)
{
    accessor acc;
    if (_entities.insert(acc, e.id)) {
        acc->second = std::move(e);
        acc->second.initRPCService();
    } else {
                    ...
                    for (auto& [id, data] : entities) {
                        try {
                            ...
                            std::cerr << "Error while loading entity " << id << ": "
                                      << e.what() << std::endl;
#endif
                            LOGERROR("Error while loading entity " + id + ": " + std::string(e.what()));
                        }
                    }
                }),
            containerId);
...
ETTRegistry::GetExistingEntities(const std::string& containerId)
{
    ...
    for (auto& [id, e] : _entities) {
        if (e.ownerContainerId == containerId) {
            ...
            j["ghost"] = GHOSTSYSTEM.PeekProperties(id).value_or(nlohmann::json::object());
...
{
    std::cout << "Entity::initRPCService: " << id << std::endl;
#ifdef CELTE_SERVER_MODE_ENABLED

    if (ContainerRegistry::GetInstance().ContainerIsLocallyOwned(ownerContainerId)) {
        EntityRPCHandlerReactor::subscribe(tp::peer(id), this);
        std::cout << "Entity::initRPCService: " << id
                  << " subscribed to peer" << std::endl;
    }
#endif
