    ...
    std::function<void()> then)
{
    ...
    if (ok) {
        ...
        LOGERROR("Error connecting client " + clientId + " to grape " + RUNTIME.GetAssignedGrape());
...
void PeerService::SubscribeClientToContainer(const std::string& clientId,
    ...
    std::function<void()> then)
{
    GRAPES.RunWithLock(RUNTIME.GetAssignedGrape(), [this, clientId, containerId, then](Grape& g) {
        if (not ContainerRegistry::GetInstance().ContainerIsLocallyOwned(
                containerId)) {
            LOGERROR("Error: container " + containerId + " is not locally owned, could not subscribe client to it.");
            return;
        }
        RUNTIME.GetPeerService().GetClientRegistry().RunWithLock(
            clientId, [&](ClientData& c) {
                ...
                RUNTIME.ScheduleAsyncIOTask([this, then, clientId, containerId,
                    ...
                    LOGINFO("Subscribing client " + clientId + " to container " + containerId);
...
{
    GRAPES.RunWithLock(RUNTIME.GetAssignedGrape(), [this, clientId, containerId](Grape& g) {
        if (not ContainerRegistry::GetInstance().ContainerIsLocallyOwned(
                containerId)) {
            LOGERROR("Error: container " + containerId + " is not locally owned, could not unsubscribe client from it.");
            return;
        }
        RUNTIME.GetPeerService().GetClientRegistry().RunWithLock(
            clientId, [&](ClientData& c) {
                ...
                RUNTIME.ScheduleAsyncIOTask([this, clientId, containerId]() {
                    LOGINFO("Unsubscribing client " + clientId + " from container " + containerId);
...
std::map<std::string, int> PeerService::GetLatency()
{
    ...
    for (const auto& g : grapes) {
        ...
        latencies[g] = std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
                           .count();
    }
    return latencies;
}
