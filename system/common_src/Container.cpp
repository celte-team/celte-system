    ...
    bool* wasCreated)
{
    ...
    std::string containerId = id.empty() ? boost::uuids::to_string(boost::uuids::random_generator()())
                                         : id;
...
void ContainerRegistry::UpdateRefCount(const std::string& containerId)
{
    ...
    if (_containers.find(acc, containerId)) {
        if (acc->second.container.use_count() == 1) {
            ...
            LOGDEBUG("Container " + containerId + " is no longer referenced and has been deleted.");
        }
    }
}
