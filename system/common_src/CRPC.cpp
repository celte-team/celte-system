...
namespace celte {
        ...
        pulsar::Consumer& consumer)
    {
        ...
        std::string subscriptionName = RUNTIME.GetUUID();
        pulsar::Result result = _client->subscribe(scope, subscriptionName, consumerConfig, consumer);
        if (result != pulsar::ResultOk) {
            throw std::runtime_error("Failed to subscribe to " + scope + ": " + pulsar::strResult(result));
        }
    }
