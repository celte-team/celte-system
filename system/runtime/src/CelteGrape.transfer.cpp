...
namespace celte {
    namespace chunks {
            ...
            const std::string& callerId)
        {
            ...
            std::cerr << "Error in __rp_remoteTakeEntity: entity not found" << std::endl;
            return false;
        }
            ...
            const std::string& newOwnerGrapeId, int tick)
        {
            ...
            RUNTIME.IO().post([this, newOwnerChunkId, tick, entityUUID, newOwnerGrapePtr]() {
                std::optional<std::shared_ptr<IEntityContainer>> newOwnerContainer = newOwnerGrapePtr->GetReplicationGraph().GetContainerOpt(
                    newOwnerChunkId);
                if (not newOwnerContainer.has_value()) {
                    newOwnerContainer = newOwnerGrapePtr->GetReplicationGraph().AddContainer(newOwnerChunkId);
                }
