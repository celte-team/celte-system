...
namespace celte {
    namespace chunks {
        ...
        void Chunk::__rp_disconnectPlayer(const std::string& entityId)
        {

            try {
                if (not ENTITIES.IsEntityRegistered(entityId)) {
                    std::cerr << "Entity " << entityId << " not found" << std::endl;
                    return;
                }
        ...
        void Chunk::TakeEntity(const std::string& entityId)
        {
            ...
            try {
                ...
                std::cerr << "Error in TakeEntity: " << e.what() << std::endl;
            }
        }
            ...
            int tick)
        {
            try {
                ...
                std::cerr << "Error in ScheduleEntityAuthorityTransfer: " << e.what()
                          << std::endl;
            }
        }
        ...
        void Chunk::__rp_containerDrops(const std::string& transferInfo, int tick)
        {
            CLOCK.ScheduleAt(tick, [this, transferInfo]() {
                ...
                try {
                    ...
                    // container not set
                    std::cerr << "Error in __rp_containerDrops: " << e.what() << std::endl;
                }
        ...
        {
            std::cout << "In System Deleting entity id : " << entityId << std::endl;
            _rpcs.CallVoid(tp::PERSIST_DEFAULT + _combinedId + "." + tp::RPCs,
                "__rp_deleteEntity", entityId);
        }
#endif

        void Chunk::__deleteEntity(const std::string& entityId)
        {
            try {
                ...
                std::cout << "Before QUA Deleting entity id : " << entity->GetUUID() << std::endl;
                ...
                entity->ExecInEngineLoop([this, entity, entityId]() {
                    ...
                    std::cout << "Before HOOK Deleting entity id : " << entity->GetUUID() << std::endl;
#ifdef CELTE_SERVER_MODE_ENABLED
