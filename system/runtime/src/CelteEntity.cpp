...
namespace celte {
    ...
    void CelteEntity::sendInputsToKafka()
    {
        ...
        if (_uuid != RUNTIME.GetUUID() || _ownerChunk == nullptr || (duration < _MsecBet2InputSend && _inputsToSend.size() < 10) || _inputsToSend.empty())
            return; // can't send inputs if not owned by a chunk
    ...
    {
        if (_ownerChunk == nullptr || _uuid != RUNTIME.GetUUID())
            return; // can't send inputs if not owned by a chunk
