...
public:
  struct OwnerContainerLock : public std::lock_guard<std::mutex> {
    OwnerContainerLock(const CelteEntity &entity)
        : std::lock_guard<std::mutex>(*entity._ownerContainerMutex) {}
  };

  inline CelteEntity() : _ownerContainerMutex(new std::mutex()) {}
   ...
   *
   * @param containerId: The id of the container that the entity is spawning
   * in.
   ...
   * current peer, and filled if the entity is owned by another peer (and
   * the spawn order is sent through the network).
   */
   ...
   * It will unregister the entity from all systems and notify all peers of
   * the event.
   */
  void OnDestroy();
  ...
  void OnChunkTakeAuthorityUnsafe(celte::chunks::Chunk &chunkId);
   ...
   */
  inline celte::chunks::Chunk &GetOwnerChunk() {
    std::lock_guard<std::mutex> lock(*_ownerContainerMutex);
    return GetOwnerChunkUnsafe();
  }

  inline celte::chunks::Chunk &GetOwnerChunkUnsafe() const {
  ...
  inline std::string GetContainerIdUnsafe() const {
    return GetOwnerChunkUnsafe().GetCombinedId();
  }
  ...
  /**
   * @brief This method is called when the entity receives data from the
   * network and should update it in order to rollback to the changes made by
   * the server that has authority over the entity.
   */
  void DownloadReplicationData(const std::string &blob);

  /**
   * @brief Send an input to kafka, this will trigger a RPC in the other
   ...
   */
  void sendInputToKafka(std::string inputName, bool pressed, float x = 0,
                        float y = 0);
...
private:
  ...
  std::shared_ptr<std::mutex> _ownerContainerMutex;
